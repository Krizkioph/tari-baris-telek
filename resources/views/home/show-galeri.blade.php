<x-layout>
    ...
    <x-page-heading>{{$foto->deskripsi}}</x-page-heading>
    ...
    <div class="flex flex-col items-start justify-between gap-3 mt-6 sm:flex-row sm:items-center">
        <div class="flex items-center space-x-2">
            ...
            <p class="text-sm">Upload oleh <span class="font-semibold ">{{$foto->admin->name}}</span></p>
        </div>
        ...
        <div class="flex items-center space-x-2">
            ...
            <p class="text-sm">{{$foto->created_at->format('d F Y')}}</p>
        </div>
    </div>
</x-layout>